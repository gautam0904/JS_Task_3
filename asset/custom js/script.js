...
$(document).ready(function () {
  ...
  $('#s-outputTable tbody').on('click', 'td .details-control', function () {
    ...
    if (row.child.isShown()) {
      ...
      row.child(formatChildTable()).show();
      tr.addClass('shown');
      const eftable = document.getElementById("formTable");
      // Get the output element for education
      const tbody = document.getElementById("o-bdy");
      tbody.innerHTML = ''; // Clear previous data

      // Iterate over each row in the table
      eftable.querySelectorAll("tr").forEach(row => {
        ...
        row.querySelectorAll("td").forEach(cell => {
          ...
          if (input) {
            ...
            outputCell.textContent = input.value;
            outputRow.appendChild(outputCell);
          }
        });
        // Append the output row to the table body
        tbody.appendChild(outputRow);
      });
    }
  });

  function formatChildTable() {
    return '   <table class="table" id="outputTable"><thead><tr><th scope="col">Degree/Board</th><th scope="col">School/College</th><th scope="col">Start Date</th><th scope="col">paasout Year</th><th scope="col">Percentage</th><th scope="col">Backlog</th></tr></thead><tbody id="o-bdy"></tbody></table>';
  }
...
function show() {
  ...
  if (t == 1) {
    ...
    if (validateForm()) {
      ...
      let formData = [
        '<td><button class="details-control">></button></td>',
        $('#first-name').val(),
        $('#last-name').val(),
        $('#dob').val(),
        $('#email').val(),
...
$('#s-outputTable').on('click', '.edit-row', function () {
  ...
  $('#first-name').val(rowData[1]);
  $('#last-name').val(rowData[2]);
  $('#dob').val(rowData[3]);
  $('#email').val(rowData[4]);
  $('#address').val(rowData[5]);
  $('#grd-year').val(rowData[6]);
  $('#shBtn').remove();
...
function updateRow() {
  ...
  if (validateForm()) {
    ...
    currentlyEditingRow.find('td:eq(0)').html('<button class="details-control">></button>')
    currentlyEditingRow.find('td:eq(1)').text($('#first-name').val());
    currentlyEditingRow.find('td:eq(2)').text($('#last-name').val());
    currentlyEditingRow.find('td:eq(3)').text($('#dob').val());
    currentlyEditingRow.find('td:eq(4)').text($('#email').val());
    currentlyEditingRow.find('td:eq(5)').text($('#address').val());
    currentlyEditingRow.find('td:eq(6)').text($('#grd-year').val());
